     ...
     public static function activeRoute($route)
    {
	 ...
	 public static function createDeveloper(){
        ...
        if(!$new_role_check){
            ...
            $new_perm_check5 = Permission::where('perm_name', 'Configure System')->count();
            if(!$new_perm_check5){
                $new_perm5 = new Permission;
                $new_perm5->perm_name  = 'Configure System';
                $new_perm5->isparent   = 0;
                $new_perm5->status     = 1;
                $new_perm5->routename  = 'configure.system.info';
                $new_perm5->faicon     = 'fa-cog';
                $new_perm5->isnav      = 1;
                $new_perm5->detail     = 'Configure System';
                $new_perm5->permParent = $np;
                $new_perm5->save();
                $new_roleperm_check5  = Rolepermission::where('role_id', $new_role->id)->where('permission_id', $new_perm5->id)->count();
                if(!$new_roleperm_check5){
                    $new_roleperm5 = new Rolepermission;
                    $new_roleperm5->role_id = $new_role->id;
                    $new_roleperm5->permission_id = $new_perm5->id;
                    $new_roleperm5->save();
                }
            }
        }
